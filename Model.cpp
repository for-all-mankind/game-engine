...
std::vector<Texture> Model::loadMaterialTextures(aiMaterial* mat, aiTextureType aiType, TextureType type)
{
	...
	for (int i = 0; i < mat->GetTextureCount(aiType); i++)
	{
		...
		Texture tex;
		tex.handle = loadModelTexture(str.C_Str(), type);
		tex.type = type;
...
{
	Texture texture;
	texture = load_texture(filename, type, TextureFilter::LinearMipmapLinear, TextureFilter::Linear, TextureWrapMode::Clamp);
